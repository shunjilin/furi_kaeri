import domain/card_v2
import domain/lane_v2
import domain/user
import domain/values/non_empty_string
import domain/vote
import gleam/dict
import gleam/list
import gleam/option
import gleam/result

pub type Board {
  Board(
    id: String,
    title: non_empty_string.NonEmptyString,
    lanes: List(lane_v2.Lane),
    phase: BoardPhase,
  )
}

pub type BoardPhase {
  DraftBoard(
    cards: dict.Dict(
      card_v2.CardId,
      #(lane_v2.LaneId, card_v2.Card(card_v2.Draft)),
    ),
  )
  RevealedBoard(
    cards: dict.Dict(
      card_v2.CardId,
      #(lane_v2.LaneId, card_v2.Card(card_v2.Revealed)),
    ),
  )
  VotingBoard(
    cards: dict.Dict(
      card_v2.CardId,
      #(lane_v2.LaneId, card_v2.Card(card_v2.Voting)),
    ),
  )
}

pub type BoardId {
  BoardId(String)
}

pub fn id(board: Board) -> String {
  board.id
}

pub fn title(board: Board) -> non_empty_string.NonEmptyString {
  board.title
}

pub fn lanes(board: Board) -> List(lane_v2.Lane) {
  board.lanes
}

pub fn new(
  id: String,
  title: non_empty_string.NonEmptyString,
  lanes: List(lane_v2.Lane),
) -> Board {
  Board(id, title, lanes:, phase: DraftBoard(dict.new()))
}

pub type AddCardError {
  AddCardLaneNotFound
  NotDraftPhase
}

pub fn add_card(
  board: Board,
  card: card_v2.Card(card_v2.Draft),
  lane_id: lane_v2.LaneId,
) -> Result(Board, AddCardError) {
  case board.phase {
    DraftBoard(cards) -> {
      use lane <- result.try(
        board.lanes
        |> list.find(fn(l) { lane_v2.id(l) == lane_id })
        |> result.replace_error(AddCardLaneNotFound),
      )

      let entry = #(lane_v2.id(lane), card)
      let operations = [#(card_v2.id(card), option.Some(entry))]
      let updated_cards = apply_updates(cards, operations)

      Ok(Board(..board, phase: DraftBoard(updated_cards)))
    }
    _ -> Error(NotDraftPhase)
  }
}

pub type UpdateCardError(error) {
  UpdateCardNotFound
  UpdateCardError(error)
  PhaseViolation
}

pub fn edit_card(
  board: Board,
  author_id: user.UserId,
  card_id: card_v2.CardId,
  content: non_empty_string.NonEmptyString,
) -> Result(Board, UpdateCardError(card_v2.EditError)) {
  case board.phase {
    DraftBoard(cards) -> {
      update_card(cards, card_id, fn(card) {
        card_v2.edit(card:, author_id:, content:) |> result.map(option.Some)
      })
      |> result.map(fn(updated) { Board(..board, phase: DraftBoard(updated)) })
    }
    _ -> Error(PhaseViolation)
  }
}

pub fn remove_card(
  board: Board,
  author_id: user.UserId,
  card_id: card_v2.CardId,
) -> Result(Board, UpdateCardError(card_v2.RemoveError)) {
  case board.phase {
    DraftBoard(cards) -> {
      update_card(cards, card_id, fn(card) {
        card_v2.remove(card:, author_id:) |> result.map(fn(_) { option.None })
      })
      |> result.map(fn(updated) { Board(..board, phase: DraftBoard(updated)) })
    }
    _ -> Error(PhaseViolation)
  }
}

pub fn vote(
  board: Board,
  vote: vote.Vote,
  card_id: card_v2.CardId,
) -> Result(Board, UpdateCardError(card_v2.VoteError)) {
  case board.phase {
    VotingBoard(cards) -> {
      update_card(cards, card_id, fn(card) {
        card_v2.vote(card:, vote:) |> result.map(option.Some)
      })
      |> result.map(fn(updated) { Board(..board, phase: VotingBoard(updated)) })
    }
    _ -> Error(PhaseViolation)
  }
}

pub fn remove_vote(
  board: Board,
  vote: vote.Vote,
  card_id: card_v2.CardId,
) -> Result(Board, UpdateCardError(card_v2.RemoveVoteError)) {
  case board.phase {
    VotingBoard(cards) -> {
      update_card(cards, card_id, fn(card) {
        card_v2.remove_vote(card:, vote:) |> result.map(option.Some)
      })
      |> result.map(fn(updated) { Board(..board, phase: VotingBoard(updated)) })
    }
    _ -> Error(PhaseViolation)
  }
}

pub type MergeCardsError {
  MergeTargetNotFound
  MergeSourceNotFound
  MergeCardError(card_v2.MergeError)
  NotRevealedPhase
}

pub fn merge_cards(
  board: Board,
  target_id: card_v2.CardId,
  source_id: card_v2.CardId,
) -> Result(Board, MergeCardsError) {
  case board.phase {
    RevealedBoard(cards) -> {
      use #(target_lane, target_card) <- result.try(
        dict.get(cards, target_id) |> result.replace_error(MergeTargetNotFound),
      )
      use #(_source_lane, source_card) <- result.try(
        dict.get(cards, source_id) |> result.replace_error(MergeSourceNotFound),
      )

      use updated_target <- result.try(
        card_v2.merge(into: target_card, from: source_card)
        |> result.map_error(MergeCardError),
      )

      let operations = [
        #(target_id, option.Some(#(target_lane, updated_target))),
        #(source_id, option.None),
      ]
      let updated_cards = apply_updates(cards, operations)

      Ok(Board(..board, phase: RevealedBoard(updated_cards)))
    }
    _ -> Error(NotRevealedPhase)
  }
}

pub type TransitionError {
  InvalidTransitionState
}

pub fn reveal(board: Board) -> Result(Board, TransitionError) {
  case board.phase {
    DraftBoard(cards) -> {
      let transitioned = transition_cards(cards, card_v2.reveal)
      Ok(Board(..board, phase: RevealedBoard(transitioned)))
    }
    _ -> Error(InvalidTransitionState)
  }
}

pub fn start_voting(board: Board) -> Result(Board, TransitionError) {
  case board.phase {
    RevealedBoard(cards) -> {
      let transitioned = transition_cards(cards, card_v2.start_voting)
      Ok(Board(..board, phase: VotingBoard(transitioned)))
    }
    _ -> Error(InvalidTransitionState)
  }
}

fn transition_cards(
  cards: dict.Dict(card_v2.CardId, #(lane_v2.LaneId, card_v2.Card(from_phase))),
  transition: fn(card_v2.Card(from_phase)) -> card_v2.Card(to_phase),
) -> dict.Dict(card_v2.CardId, #(lane_v2.LaneId, card_v2.Card(to_phase))) {
  dict.fold(over: cards, from: dict.new(), with: fn(acc_dict, card_id, entry) {
    let #(lane_id, card) = entry
    dict.insert(acc_dict, card_id, #(lane_id, transition(card)))
  })
}

fn update_card(
  cards: dict.Dict(card_v2.CardId, #(lane_v2.LaneId, card_v2.Card(phase))),
  card_id: card_v2.CardId,
  transform: fn(card_v2.Card(phase)) ->
    Result(option.Option(card_v2.Card(phase)), error),
) -> Result(
  dict.Dict(card_v2.CardId, #(lane_v2.LaneId, card_v2.Card(phase))),
  UpdateCardError(error),
) {
  use #(lane_id, card_to_update) <- result.try(
    dict.get(cards, card_id) |> result.replace_error(UpdateCardNotFound),
  )

  use updated_card <- result.try(
    transform(card_to_update) |> result.map_error(UpdateCardError),
  )

  let operations = [
    #(card_id, option.map(updated_card, fn(card) { #(lane_id, card) })),
  ]
  Ok(apply_updates(cards, operations))
}

fn apply_updates(
  cards: dict.Dict(card_v2.CardId, #(lane_v2.LaneId, card_v2.Card(phase))),
  operations: List(
    #(card_v2.CardId, option.Option(#(lane_v2.LaneId, card_v2.Card(phase)))),
  ),
) -> dict.Dict(card_v2.CardId, #(lane_v2.LaneId, card_v2.Card(phase))) {
  list.fold(over: operations, from: cards, with: fn(current_cards, operation) {
    let #(id, decision) = operation
    case decision {
      option.Some(updated_entry) ->
        dict.insert(current_cards, id, updated_entry)
      option.None -> dict.delete(current_cards, id)
    }
  })
}
